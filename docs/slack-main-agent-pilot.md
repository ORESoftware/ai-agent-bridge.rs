...
The reviewed manifest also defines these convenience aliases; they share the two canonical request URLs:
...
`/x-claude` and `/my-claude` share the canonical Claude endpoint. `/x-chatgpt` and `/my-chatgpt` share the canonical ChatGPT endpoint. Slack includes the actual command name in the signed form payload, and the runtime rejects any command whose provider disagrees with the path.
