...
mod block_kit_limit_tests {
    ...
    use crate::slack_project_bindings::{AgentMode, BudgetPolicy, ChannelProjectBinding, WritePolicy};
    ...
    fn binding(repositories: usize, write_policy: WritePolicy) -> ChannelProjectBinding {
        let repository_allowlist: BTreeSet<String> = (0..repositories)
            ...
            .collect();
        let default_repository = repository_allowlist.iter().next().cloned()
            .unwrap_or_else(|| "oresoftware/repo-000".to_string());
        ChannelProjectBinding {
            workspace_id: "T01B3C83PMK".into(), channel_id: "C0BKP2N3LG7".into(),
            linear_team_id: "team-uuid".into(), linear_team_key: "DEN".into(),
            linear_project_id: "project-uuid".into(), default_repository, repository_allowlist,
            default_agent_mode: AgentMode::Claude,
            allowed_agent_modes: [AgentMode::Claude, AgentMode::Chatgpt].into_iter().collect(),
            allowed_user_ids: ["U1".to_string()].into_iter().collect(),
            allowed_user_group_ids: BTreeSet::new(), write_policy,
            budget_policy: BudgetPolicy { max_concurrent_runs: 2, max_runtime_secs: 600, max_tokens: 100_000, max_spend_cents: 500, max_retries: 2 },
            updated_by: "U1".into(), updated_at: "2026-08-01T12:00:00Z".into(),
        }
    }

    fn assert_within_block_kit_limits(view: &Value) {
        let title = view["title"]["text"].as_str().expect("title");
        assert!(title.chars().count() <= 24, "modal title {title:?} exceeds 24 characters");
        for key in ["submit", "close"] {
            let label = view[key]["text"].as_str().expect(key);
            assert!(label.chars().count() <= 24, "{key} label {label:?} exceeds 24 characters");
        }
        ...
        for block in blocks {
            ...
            assert!(block["label"]["text"].as_str().is_some_and(|label| label.chars().count() <= 2_000), "{block_id} label exceeds 2000 characters");
            ...
            if let Some(max_length) = element["max_length"].as_u64() {
                assert!(max_length <= 3_000, "{block_id} plain_text_input max_length exceeds 3000");
            }
            if let Some(options) = element["options"].as_array() {
                ...
                for option in options {
                    ...
                    assert!(text.chars().count() <= 75, "{block_id} option label too long: {text}");
                    assert!(value.len() <= 150, "{block_id} option value too long");
                }
                if let Some(initial) = element.get("initial_option") {
                    assert!(options.contains(initial), "{block_id} preselects an option absent from its menu: {initial}");
                }
            }
        }
    }
    ...
    fn every_modal_shape_respects_slack_block_kit_limits() {
        ...
        for provider in [Provider::Claude, Provider::Chatgpt] {
            for write_policy in [WritePolicy::ReadOnly, WritePolicy::LinearOnly, WritePolicy::DraftPullRequest] {
                for context_messages in [0, 5, 10, 20] {
                    assert_within_block_kit_limits(&modal(provider, &binding(100, write_policy), &metadata, context_messages));
                }
            }
        }
    }
    ...
    fn the_repository_menu_never_drops_the_default_repository() {
        ...
        let repository_block = view["blocks"].as_array().expect("blocks").iter()
            .find(|block| block["block_id"] == "repository").expect("repository block");
        let options = repository_block["element"]["options"].as_array().expect("options");
        assert!(options.iter().any(|option| option["value"] == binding.default_repository.as_str()), "the default repository must remain selectable");
    }
}
