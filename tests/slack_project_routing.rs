...
use fiducia_ai_agent_bridge::slack_project_bindings::{
...
const CHANNELS: [(&str, &str, &str, &str); 13] = [
    ...
    ("shared-auth", "CSHAREDAUTH", "shared-auth", "shared-auth-clients"),
...
fn agent_modes_outside_a_channels_allowlist_are_refused() {
    ...
    for mode in [AgentMode::Claude, AgentMode::Chatgpt] {
        ...
        assert_eq!(registry.resolve(&ask).expect("allowed mode").agent_mode, mode);
    }
...
) -> fiducia_ai_agent_bridge::slack_project_bindings::LinearIssueRef {
    registry
        .resolve(&request)
        .expect("resolve")
        .issue
